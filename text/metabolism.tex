...
Control theory provides a theoretical foundation underpinning the response of systems displaced from steady state. One of its core principles is the notion of Lyapunov stability; that is, systems tend to remain near stable equilibria. Another is the Hartman-Grobman theorem, which posits that systems deviate approximately linearly about these fixed points \cite{Arrowsmith1992}. Models based on these principles seek to describe how system output deviates from its steady state value in time. Describing our model with control terminology, protein level remains fixed about a basal steady state. Deviations from the basal level are driven by a transient disturbance. The disturbance induces activation of a gene, which induces transcription of mRNAs, which proceed to induce translation of protein. These three state variables are linearized about their steady state values:
\begin{equation}
...
\caption[Robustness of reduced metabolism simulations to model assumptions.]{\textbf{Reduced energy metabolism diminishes the importance of auxiliary repressors over a wide range of model conditions.} Each panel depicts a parameter sweep of the nine-dimensional hyperspace defined by one order of magnitude variation in each of the default model parameters. For each parameter set, error frequencies pertain to 50\% loss of repression. (A) Distribution of error frequencies for all parameter sets under conditions of reduced energy metabolism. (B) Distribution of differential error frequencies relative to normal metabolic conditions, e.g. blue indicates error suppression by reduced energy metabolism. (C) Results from (B) re-calculated across a range of different success thresholds. Each line corresponds to a single parameter set. Color scale reflects maximum change in error frequency across the threshold range. Black dashed line denotes the absence of a rescue effect. Most simulations exhibit some reduction in error frequency across all thresholds. (D-G) Distributions of differential error frequencies relative to normal metabolic conditions when (D) a nonzero basal stimulus is applied, (E) input duration is increased four-fold by a reduction in energy metabolism, (F) an upper bound is placed on the number of sites firing transcription, and (G) cooperative transcription kinetics are considered. Blue-Red color scale corresponds to the difference in error frequency between slow and normal conditions, e.g. blue indicates error suppression by reduced energy metabolism.}
...
\caption[Robustness of ribosomopathy simulations to model assumptions.]{\textbf{Reduced protein synthesis capacity diminishes the importance of auxiliary repression over a wide range of model conditions.} Each panel depicts a parameter sweep of the nine-dimensional hyperspace defined by one order of magnitude variation in each of the default model parameters. For each parameter set, error frequencies pertain to 50\% loss of repression. (A-C) Simulations in which the duration of the stimulus is unaffected by changes in protein synthesis. Shown are the (A) differential error frequencies and (B) differential changes in expression dynamics relative to normal protein synthesis conditions. (C) Differential error frequencies for varying definitions of the success threshold. Each line represents one parameter set, colored by the corresponding range of differential error frequencies. (D) Simulations where a nonzero basal stimulus is applied. (E) Simulations where input duration is increased two-fold by a reduction in protein synthesis capacity. (F) Simulations when an upper bound is placed on the number of sites firing transcription. (G) Simulations when cooperative transcription kinetics are considered.}
